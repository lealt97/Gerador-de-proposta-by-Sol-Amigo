...
import { buildSolarKitSnapshot, SolarKit } from '../../../types/solarKit';
import { AlertCircle, CheckCircle2, Loader2, Package, Plus, RefreshCw, Trash2 } from 'lucide-react';
...
const resolveRequiredKwp = (values: Partial<ProposalFormValues>) => {
  ...
  if (projectedConsumption <= 0 || hsp <= 0 || yieldFactor <= 0) {
    return 0;
  }
...
export function StepCosts() {
  const { register, control, setValue, getValues } = useFormContext<ProposalFormValues>();
  ...
  const recommendedKit = useMemo(() => solarKitService.recommendKit(kits, requiredKwp), [kits, requiredKwp]);
  const selectedKit = useMemo(
    () => kits.find((kit) => kit.id === selectedSolarKitId) || null,
    [kits, selectedSolarKitId]
  );
      ...
      <div className="rounded-xl border border-brand-border bg-brand-surface p-4">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              Escolha um kit cadastrado para preencher automaticamente o custo do kit e os dados de módulo.
            </p>
          </div>
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 lg:grid-cols-[1fr_auto] lg:items-end">
            ...
            >
              ...
              {kits.map((kit) => (
                <option key={kit.id} value={kit.id}>
                  {kit.name} — {Number(kit.kit_power_kwp || 0).toFixed(2)} kWp — {formatMoney(Number(kit.cost_price || 0))}
                </option>
              ))}
            </select>
          </div>
          ...
          <Button
            ...
            variant="outline"
            className="gap-2"
            disabled={!recommendedKit || isLoadingKits}
            onClick={() => recommendedKit && applyKitToProposal(recommendedKit)}
          >
            ...
            <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  {Number(recommendedKit.kit_power_kwp || 0).toFixed(2)} kWp · {recommendedKit.module_quantity} módulos de {recommendedKit.module_power_w} W
        ...
        {selectedKit && (
          <div className="mt-3 flex flex-col gap-2 rounded-lg border border-brand-border bg-gray-50 p-3 text-xs text-slate-500 md:flex-row md:items-center md:justify-between">
            <span>
              Kit aplicado: <strong className="text-brand-dark">{selectedKit.name}</strong> · custo lançado em Custo do Kit.
            </span>
            <Button type="button" variant="ghost" size="sm" onClick={clearSelectedKit}>
              Remover vínculo do kit
            </Button>
          </div>
        )}
      </div>
